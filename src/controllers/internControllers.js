...
const createIntern = async function (req, res) {
    try {
        ...
        if (requestBody.email) {
            let validmail = /^\w+([\.-]?\w+)@\w+([\. -]?\w+)(\.\w{2,3})+$/.test(requestBody.email)
            if (!validmail) {
        ...
        if (isEmailAlreadyUsed) {
            res.status(400).send({ status: false, message: `${email} Email is already used, try different one ` })
            return
        }
        if (!isValid(mobile)) {
        ...
        if (ismobileAlreadyUsed) {
            res.status(400).send({ status: false, message: `${mobile} mobile is already used, try different one` })
            return
        }
